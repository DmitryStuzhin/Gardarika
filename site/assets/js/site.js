...
function showQ(i){
  ...
  qBar.style.transform = "scaleX(" + (last ? 1 : (i + 1) / (qs.length - 1)) + ")";
  qBack.hidden = i === 0;
}
function quizResult(){
...
$("#quizToContact").addEventListener("click", function(){ topic.value = this.getAttribute("data-topic") || ""; });
...
function route(){
  ...
  if (mc){ location.replace("calculator.html" + (mc[1] ? "#" + mc[1] : "")); return; }
...
(function(){
  ...
  function upd(){ bar.classList.toggle("is-on", window.scrollY > hero.offsetHeight * .6); }
