(function () {
  ...
  [[-7,-6,1.2],[-8,2,.8],[7,5,1.05],[8,-1,.72],[-6,7,.62]].forEach(function(v,i){var trunk=new T.Mesh(new T.CylinderGeometry(.16,.23,2.2,8),trunkMat);trunk.position.set(v[0],1,v[1]);trunk.castShadow=true;site.add(trunk);[[-.38,0,.08],[.38,.12,-.08],[0,.48,0]].forEach(function(o,j){var crown=new T.Mesh(new T.SphereGeometry((.86-j*.06)*v[2],14,10),j===1?leafLightMat:leafMat);crown.scale.set(1.1,1.28,.98);crown.position.set(v[0]+o[0]*v[2],2.45+o[1]*v[2],v[1]+o[2]);crown.castShadow=true;site.add(crown);});});
  [[-4.7,-4.1],[4.7,2.8],[-3.8,5.2],[3.8,5.35],[-5.1,1.8]].forEach(function(v,i){for(var j=0;j<3;j++){var bush=new T.Mesh(new T.SphereGeometry(.32+j*.04,10,7),j%2?leafLightMat:leafMat);bush.scale.y=.72;bush.position.set(v[0]+j*.38,.24,v[1]+(j%2)*.22);bush.castShadow=true;site.add(bush);}});
